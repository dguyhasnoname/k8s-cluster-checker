...
If you want the json data generated by this tool to be ingested in Splunk for dashboard,  please user the [CronJob.yaml](/CronJob.yaml) to deploy k8s-cluster-checker in your cluster. Docker image used for this  purpose runs the tool  with `-l`  flag which only generated JSON data on stdout.
